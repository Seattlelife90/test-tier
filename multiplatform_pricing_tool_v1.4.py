# multiplatform_pricing_tool_v1.4.py
...
# ============================================================================
# PLAYSTATION PRICE PULLER (Using your exact working code)
# ============================================================================
...
def from_next_json(next_json: dict) -> Tuple[Optional[str], Optional[str], Optional[float], Optional[float], Optional[str]]:
    """Extract price data from PlayStation Next.js JSON - PRIORITIZE basePrice (MSRP)"""
    try:
        ...
        # Location 1: Direct product.price - PRIORITIZE basePrice
        p = product.get("price") if isinstance(product, dict) else None
        if isinstance(p, dict):
            msrp = _num(p.get("basePrice"))  # THIS IS MSRP - prioritize this
            if msrp is None or msrp == 0:
                msrp = _num(p.get("discountedPrice") or p.get("finalPrice") or p.get("current") or p.get("value"))
            current = msrp  # Use base price as current
        ...
        if current is None:
            ...
            if isinstance(default_sku, dict):
                ...
                if p2:
                    msrp = _num(p2.get("basePrice"))
                    if msrp is None or msrp == 0:
                        msrp = _num(p2.get("discountedPrice") or p2.get("finalPrice") or p2.get("current") or p2.get("value"))
                    current = msrp
        ...
        if current is None:
            ...
            if isinstance(skus, list):
                for sku in skus:
                    ...
                    if p3:
                        msrp = _num(p3.get("basePrice"))
                        if msrp is None or msrp == 0:
                            msrp = _num(p3.get("discountedPrice") or p3.get("finalPrice") or p3.get("current") or p3.get("value"))
                        current = msrp
        ...
        if current is None:
            ...
            if isinstance(pp, dict):
                msrp = _num(pp.get("basePrice"))
                if msrp is None or msrp == 0:
                    msrp = _num(pp.get("discountedPrice") or pp.get("finalPrice") or pp.get("current"))
                current = msrp
...
def fetch_ps_price(product_id: str, country: str, title: str) -> Optional[PriceData]:
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if nxt:
                t, _, m, c, pcurr = from_next_json(nxt)
                if c is not None and c > 0:
                    final_currency = choose_currency(pcurr, currency)
                    return PriceData("PlayStation", title, country, final_currency, c, None, None)
            
            # Fallback to JSON-LD
            t2, c2, pcurr2 = parse_json_ld(html)
            if c2 is not None and c2 > 0:
                final_currency = choose_currency(pcurr2, currency)
                return PriceData("PlayStation", title, country, final_currency, c2, None, None)
            
            # Fallback to meta tags
            c3, pcurr3 = parse_meta_tags(html)
            if c3 is not None and c3 > 0:
                final_currency = choose_currency(pcurr3, currency)
                return PriceData("PlayStation", title, country, final_currency, c3, None, None)
