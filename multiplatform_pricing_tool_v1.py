...
def parse_ps_next_json(html: str) -> Tuple[Optional[str], Optional[float], Optional[str]]:
    ...
    try:
        ...
        price = None
        for key in ["discountedPrice", "finalPrice", "current", "value", "basePrice"]:
            if key in price_obj and price_obj[key]:
...
with col1:
    steam_input = st.text_area(
        "Steam URLs or AppIDs (one per line)",
        height=150,
        placeholder="https://store.steampowered.com/app/1449110/\n1449110"
    )
with col2:
    st.markdown("**Examples:**")
    st.code("1449110", language="text")
    st.code("store.steampowered.com/app/1449110/", language="text")
...
with col3:
    xbox_input = st.text_area(
        "Xbox URLs or Store IDs (one per line)",
        height=150,
        placeholder="https://www.xbox.com/en-US/games/store/game/9NSPRSXXZZLG\n9NSPRSXXZZLG"
    )
with col4:
    st.markdown("**Examples:**")
    st.code("9NSPRSXXZZLG", language="text")
    st.code("xbox.com/games/store/.../9NSPRSXXZZLG", language="text")
...
with col5:
    ps_input = st.text_area(
        "PlayStation URLs or Product IDs (one per line)",
        height=150,
        placeholder="https://store.playstation.com/en-us/product/UP6312-PPSA24588_00-0872768154966924\nUP6312-PPSA24588_00-0872768154966924"
    )
with col6:
    st.markdown("**Examples:**")
    st.code("UP6312-PPSA24588_00-...", language="text")
    st.code("store.playstation.com/.../product/...", language="text")
