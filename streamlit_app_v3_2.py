...
# v3.2 — Clean UX + Safe import of v2_5 without rendering its UI
import types

import streamlit as st

# --- Patch: import v2_5 with a no-op Streamlit to avoid rendering its UI on import ---
_real_st = st

class _NoOp:
    def __init__(self):
        # sidebar returns another _NoOp so attribute chaining works
        self.sidebar = self
...
# Temporarily replace streamlit module in sys.modules
import sys
sys.modules['streamlit'] = _NoOp()

# Now import v2_5 (it will see the no-op streamlit and won't render UI)
import multiplatform_pricing_tool_v2_5 as v25

# Restore real streamlit for our app
...
st.set_page_config(page_title="Game Pricing — v3.2 (Clean UX)", page_icon="🎮", layout="wide")
...
st.caption("Paste **URLs only**. Steam, Xbox, and PlayStation are fully siloed with their own markets & currencies from v2.5.")
...
def run_fetch(which: str):
    ...
    # Pulls use v2_5's siloed market maps; our import prevented its UI from rendering
