# multiplatform_pricing_tool_v2.1.py
# HYBRID APPROACH: HTML scraping + aggressive regex price hunting
# - Searches entire HTML for basePrice patterns
# - Extracts prices from inline JavaScript
# - Looks for price objects in any script tag
# - Falls back to JSON-LD if needed
...
st.set_page_config(
    page_title="Unified Game Pricing Tool v2.1",
    page_icon="🎮",
    layout="wide"
)
...
# ============================================================================
# PLAYSTATION - v2.1 AGGRESSIVE REGEX PRICE HUNTING
# ============================================================================
...
def hunt_prices_in_html(html: str) -> List[Dict[str, Any]]:
    ...
    # Pattern 1: Search for basePrice:"$XX.XX" or basePrice:"€XX.XX"
...
def fetch_ps_price(product_id: str, country: str, title: str) -> Optional[PriceData]:
    """v2.1: Aggressive price hunting across entire HTML"""
        ...
        if html_findings:
            ...
            for finding in html_findings:
                ...
                if base and base > 0:
                    ...
                    print(f"[PSN Regex {country}] {title}: {base} {curr} (MSRP from {finding['source']})")
...
st.title("🎮 Unified Multi-Platform Game Pricing Tool v2.1")
st.caption("🔍 AGGRESSIVE: Regex price hunting across entire HTML - finds basePrice anywhere in the page!")

if DEBUG_MODE:
    st.info("⚡ **v2.1 Hybrid Approach:** Regex searches entire HTML for 'basePrice' patterns, then falls back to JSON-LD")
...
if st.button("🚀 Pull Prices", type="primary", use_container_width=True):
    ...
    if not steam_games and not xbox_games and not ps_games:
        ...
        st.caption("🔍 PlayStation v2.1: Using aggressive regex to hunt for basePrice in entire HTML")
        ...
        if ps_results:
            ...
            if DEBUG_MODE:
                st.markdown("**📊 v2.1 Regex Hunt Results:**")
                ...
                if msrp_count > 0:
                    st.success(f"🎉 Regex hunt successfully found {msrp_count} MSRP prices in HTML!")
                ...
                st.caption("📝 Check 'Debug Info' for regex match details and 'Source' for extraction method")
