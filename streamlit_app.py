...
# - Applies vanity pricing rules (AU/NZ: xx9.95; CA: xx9.99)
...
st.caption("Live pulls from Steam (Store API) and Xbox Store. HELLDIVERS 2 is scaled by (price/4)*7 to normalize to a $69.99 basket.")
...
VANITY_RULES = {
    "AU": {"suffix": 0.95, "nines": True},
    "NZ": {"suffix": 0.95, "nines": True},
    "CA": {"suffix": 0.99, "nines": True},
}
...
def fetch_steam_price(appid: str, cc: str, forced_title: Optional[str] = None) -> Optional[PriceRow]:
    try:
        ...
        name = forced_title or (data.get("data") or {}).get("name") or f"Steam App {appid}"
        return PriceRow("Steam", name, cc.upper(), str(currency).upper() if currency else None, price, f"https://store.steampowered.com/app/{appid}")
    except Exception:
        return None
...
def fetch_xbox_price(product_name: str, product_id: str, market: str) -> Optional[PriceRow]:
    ...
    for pid in ids_to_try:
        ...
        if got:
            amount, ccy = got
            return PriceRow("Xbox", product_name, market.upper(), ccy.upper() if ccy else None, float(amount), f"https://www.xbox.com/{loc.split('-')[0]}/games/store/placeholder/{pid}")
    for pid in ids_to_try:
        got = fetch_xbox_price_one_market(pid, market=market, locale="en-US")
        if got:
            amount, ccy = got
            return PriceRow("Xbox", product_name, market.upper(), ccy.upper() if ccy else None, float(amount), f"https://www.xbox.com/en-US/games/store/placeholder/{pid}")
    return None
...
with st.sidebar:
    ...
    user_markets = st.text_area("Markets (comma-separated ISO country codes)", value=default_markets_text, height=120)
...
if run:
        ...
        with ThreadPoolExecutor(max_workers=14) as ex:
            for cc in markets:
                for title in BASKET_TITLES:
                    ...
                    if appid:
                        futures.append(ex.submit(fetch_steam_price, appid, cc, title))
            for cc in markets:
                for title in BASKET_TITLES:
                    pid = XBOX_PRODUCT_IDS.get(title)
                    if pid:
                        futures.append(ex.submit(fetch_xbox_price, title, pid, cc))
    ...
    merged = pd.merge(reco_xbox.rename(columns={"RecommendedPrice": "XboxRecommended"}), reco_steam.rename(columns={"RecommendedPrice": "SteamRecommended"}), on=["country", "currency"], how="outer").sort_values(["country"]).reset_index(drop=True)
    st.subheader("Combined Recommendations (Xbox + Steam)")
    st.dataframe(merged)
    csv = merged.to_csv(index=False).encode("utf-8")
    st.download_button(label="⬇️ Download CSV (combined recommendations)", data=csv, file_name="aaa_tier_recommendations_xbox_steam.csv", mime="text/csv")
else:
