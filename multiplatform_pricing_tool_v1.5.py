# multiplatform_pricing_tool_v1.5.py
# FIXED: PlayStation MSRP priority logic + Debug columns
# Now correctly prioritizes basePrice (MSRP) over discounted/sale prices
...
st.set_page_config(
    page_title="Unified Game Pricing Tool v1.5",
    page_icon="🎮",
    layout="wide"
)
...
class PriceData:
    ...
    price_type: Optional[str] = "API"  # MSRP, Sale Price, Current, API
    source: Optional[str] = None  # next_json, json_ld, meta, api
...
# ============================================================================
# PLAYSTATION PRICE PULLER - v1.5 FIXED MSRP PRIORITY
# ============================================================================
...
def extract_price_with_type(price_dict: dict) -> Tuple[Optional[float], str]:
    """
    Extract price from PlayStation price object with STRICT MSRP priority
    Returns: (price, price_type)
    
    Priority:
    1. basePrice (MSRP) - ALWAYS use if available
    2. finalPrice - Use if basePrice missing
    3. discountedPrice - ONLY if nothing else available
    4. current/value - Last resort
    """
    ...
    # Priority 2: finalPrice (might be MSRP or current price)
    final = _num(price_dict.get("finalPrice"))
    if final and final > 0:
        return final, "Current"
    
    # Priority 3: discountedPrice (this is a SALE price - avoid if possible)
    ...
    for key in ["current", "value"]:
        val = _num(price_dict.get(key))
        if val and val > 0:
            return val, "Current"
    
    return None, "Unknown"

def from_next_json(next_json: dict) -> Tuple[Optional[str], Optional[str], Optional[float], Optional[str], Optional[str]]:
    """
    Extract price data from PlayStation Next.js JSON
    Returns: (title, product_id, price, price_type, currency)
    """
    try:
        ...
        if isinstance(p, dict):
            price, price_type = extract_price_with_type(p)
            currency = p.get("currency") or currency
            if price and price > 0:
                return title, product_id, price, price_type, currency
        ...
        if isinstance(default_sku, dict):
            ...
            if p2:
                price, price_type = extract_price_with_type(p2)
        ...
        if isinstance(skus, list):
            for sku in skus:
                ...
                if p3:
                    price, price_type = extract_price_with_type(p3)
        ...
        if isinstance(pp, dict):
            price, price_type = extract_price_with_type(pp)
...
def fetch_ps_price(product_id: str, country: str, title: str) -> Optional[PriceData]:
    """Fetch PlayStation price for a specific country with MSRP priority"""
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if nxt:
                t, _, price, price_type, pcurr = from_next_json(nxt)
...
st.title("🎮 Unified Multi-Platform Game Pricing Tool v1.5")
st.caption("✅ FIXED: PlayStation MSRP Priority Logic | 🔍 Debug Mode: Shows Price Type & Source")

if DEBUG_MODE:
    st.info("🔍 **Debug Mode Active** - Additional columns show 'Price Type' (MSRP/Sale/Current) and 'Source' (API/Parser used)")
