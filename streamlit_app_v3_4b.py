...
# v3.4b — Hardened output (case-insensitive columns, graceful fallbacks), Clean UX + Autorun
import sys
...
st.set_page_config(page_title="Game Pricing — v3.4b (Clean UX, Autorun)", page_icon="🎮", layout="wide")
...
def recommendations_mean_mode(df: pd.DataFrame) -> pd.DataFrame:
    ...
    usd_col = cols.get("usd") or next((c for c in df.columns if c.lower()=="usd"), None)
    country_col = cols.get("country") or next((c for c in df.columns if c.lower()=="country"), None)
    platform_col = cols.get("platform") or next((c for c in df.columns if c.lower()=="platform"), None)
    if not (usd_col and country_col and platform_col):
        return pd.DataFrame(columns=["Platform","Country","Mean (USD)","Mode (USD)"])
    rows = []
...
st.title("Game Pricing Recommendation Tool — v3.4b (Xbox · Steam · PlayStation)")
...
def platform_df(df: pd.DataFrame, name: str) -> pd.DataFrame:
    if df.empty:
        return df
    # Case-insensitive detection of 'Platform' column
    colmap = {c.lower(): c for c in df.columns}
    plat_col = colmap.get("platform")
    if not plat_col:
        # No platform column — just return whole df to avoid KeyError
        return df
    return df[df[plat_col] == name]
...
with tabs[0]:
    ...
    if show_diag and not sdf.empty:
        dbg_cols = [c for c in sdf.columns if "debug" in c.lower() or "source" in c.lower() or "type" in c.lower()]
        if dbg_cols:
            st.caption("Diagnostics")
            keep_cols = [c for c in ["Platform","Title","Country"] if c in sdf.columns]
            st.dataframe(sdf[keep_cols + dbg_cols], use_container_width=True)
...
with tabs[1]:
    ...
    if show_diag and not xdf.empty:
        dbg_cols = [c for c in xdf.columns if "debug" in c.lower() or "source" in c.lower() or "type" in c.lower()]
        if dbg_cols:
            st.caption("Diagnostics")
            keep_cols = [c for c in ["Platform","Title","Country"] if c in xdf.columns]
            st.dataframe(xdf[keep_cols + dbg_cols], use_container_width=True)
...
with tabs[2]:
    ...
    if show_diag and not pdf.empty:
        dbg_cols = [c for c in pdf.columns if "debug" in c.lower() or "source" in c.lower() or "type" in c.lower()]
        if dbg_cols:
            st.caption("Diagnostics")
            keep_cols = [c for c in ["Platform","Title","Country"] if c in pdf.columns]
            st.dataframe(pdf[keep_cols + dbg_cols], use_container_width=True)

with tabs[3]:
    st.subheader("Recommendations (Mean & Mode per Platform/Country)")
    rec_df = recommendations_mean_mode(df_all) if isinstance(df_all, pd.DataFrame) and not df_all.empty else pd.DataFrame(columns=["Platform","Country","Mean (USD)","Mode (USD)"])
