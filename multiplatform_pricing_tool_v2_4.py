# multiplatform_pricing_tool_v2.4.py
# CRITICAL FIXES based on PS Deals reference:
# - ✅ CORRECT DECIMAL CURRENCIES: Only EUR/USD/GBP/CAD/AUD/NZD/CHF/SGD/ZAR/MYR
# - ✅ ALL OTHER CURRENCIES: Keep as whole numbers (CZK=1899, TRY=2900, HKD=549, etc.)
# - ✅ USD SANITY CHECK: Verify converted price is $40-$150 range
# - ✅ FIXED STEAM EU MARKETS: Each country gets own code
# - ✅ PlayStation ONLY changes (Steam/Xbox unchanged and working)
...
st.set_page_config(
    page_title="Unified Game Pricing Tool v2.4",
    page_icon="🎮",
    layout="wide"
)
...
# FIXED: Each EU country gets own Steam market code
STEAM_MARKETS = {
...
PLATFORM_CURRENCIES = {
    ...
    "PlayStation": {
        ...
        "KR": "KRW", "TW": "TWD", "HK": "HKD", "SG": "SGD", "MY": "MYR",
        "TH": "THB", "ID": "IDR", "PH": "PHP", "VN": "VND", "IN": "INR",
        "AU": "AUD", "NZ": "NZD", "ZAR": "ZAR", "UA": "UAH"
    }
}

# Based on PS Deals reference: ONLY these currencies use XX.99 format
DECIMAL_CURRENCIES = {"EUR", "USD", "GBP", "CAD", "AUD", "NZD", "CHF", "SGD", "ZAR", "MYR"}
...
def smart_price_conversion(raw_value: Any, currency: str) -> Optional[float]:
    """
    v2.4 CORRECT CONVERSION based on PS Deals reference:
    
    DECIMAL CURRENCIES (EUR, USD, GBP, CAD, AUD, NZD, CHF, SGD, ZAR, MYR):
    - If we see 7999 without decimal → convert to 79.99
    - If we see 79.99 with decimal → keep as 79.99
    
    ALL OTHER CURRENCIES (CZK, TRY, HKD, TWD, DKK, NOK, SEK, PLN, THB, INR, etc.):
    - If we see 1899 → keep as 1899 (correct for CZK)
    - If we see 2900 → keep as 2900 (correct for TRY)
    - If JSON-LD wrongly returns "18.99" for CZK → multiply by 100 to get 1899
    """
    try:
        ...
        # Check if original string had a decimal point
        has_decimal = '.' in original_str
        
        # DECIMAL CURRENCIES: EUR, USD, GBP, CAD, AUD, NZD, CHF, SGD, ZAR, MYR
        if currency in DECIMAL_CURRENCIES:
            if has_decimal:
                # Already has decimal, keep it
                return round(value, 2)
            else:
                # No decimal, check if it needs conversion
                # If value >= 1000, likely missing decimal (7999 → 79.99)
                if value >= 1000:
                    return round(value / 100.0, 2)
                else:
                    # Small value without decimal (like 79) - keep as-is
                    return round(value, 2)
        
        # ALL OTHER CURRENCIES: Keep as whole numbers
        else:
            if has_decimal:
                # JSON-LD wrongly formatted with decimal (e.g., "18.99" for CZK)
                # This should be 1899, so multiply by 100
                return round(value * 100, 0)
            else:
                # No decimal, keep as whole number
...
# ============================================================================
# PLAYSTATION - v2.4 WITH CORRECT CURRENCY HANDLING
# ============================================================================
...
def hunt_prices_in_html(html: str, currency: str) -> List[Dict[str, Any]]:
    """
    v2.4: Search for basePrice/discountedPrice with smart currency handling
    """
    findings = []
    
    # Pattern to capture price with or without decimal
    base_pattern = r'basePrice["\s:]+(["\$€£¥₹R\s]*)([\d,]+\.?\d*)'
    base_matches = re.findall(base_pattern, html, re.IGNORECASE)
    
    disc_pattern = r'discountedPrice["\s:]+(["\$€£¥₹R\s]*)([\d,]+\.?\d*)'
    ...
    for match in json_matches:
        try:
            base_search = re.search(r'basePrice["\s:]+(["\$€£¥₹R\s]*)([\d,]+\.?\d*)', match)
            disc_search = re.search(r'discountedPrice["\s:]+(["\$€£¥₹R\s]*)([\d,]+\.?\d*)', match)
            ...
            if base_search or disc_search:
                ...
                finding = {
                    'basePrice': smart_price_conversion(raw_base, detected_currency) if raw_base else None,
                    'discountedPrice': smart_price_conversion(raw_disc, detected_currency) if raw_disc else None,
    ...
    if base_matches or disc_matches:
        ...
        finding = {
            'basePrice': smart_price_conversion(raw_base, currency) if raw_base else None,
            'discountedPrice': smart_price_conversion(raw_disc, currency) if raw_disc else None,
...
def fetch_ps_price_attempt(product_id: str, country: str, title: str, currency_code: str) -> Optional[PriceData]:
    ...
    if html_findings:
        ...
        for finding in html_findings:
            ...
            if base and base > 0:
                ...
                print(f"[PSN v2.4 {country}] {title}: {base} {curr} (MSRP)")
    ...
    if script_findings:
        ...
        for finding in script_findings:
            ...
            if base and base > 0:
                ...
                full_debug = "|".join(debug_log)
                print(f"[PSN v2.4 Script {country}] {title}: {base} {curr}")
    ...
    if price2 is not None and price2 > 0:
        ...
        full_debug = "|".join(debug_log)
        print(f"[PSN v2.4 JSON-LD {country}] {title}: {price2} {curr2 or currency_code}")
...
def fetch_ps_price(product_id: str, country: str, title: str) -> Optional[PriceData]:
    """v2.4: PlayStation with correct currency handling + retry"""
    ...
    try:
        ...
        print(f"[PSN v2.4 {country}] {title}: FAILED - {debug_info}")
...
def process_results(results: List[PriceData], rates: Dict[str, float]) -> pd.DataFrame:
    ...
    # IMPORTANT: Calculate USD prices for all results
    for r in results:
...
st.title("🎮 Unified Multi-Platform Game Pricing Tool v2.4")
st.caption("✅ CORRECT: EUR/USD/GBP decimal fix + CZK/TRY/HKD whole numbers + USD conversion working!")

if DEBUG_MODE:
    st.info("🎯 **v2.4 Based on PS Deals:** Only decimal currencies get XX.99 format, all others kept as whole numbers")
...
if st.button("🚀 Pull Prices", type="primary", use_container_width=True):
    ...
    if not steam_games and not xbox_games and not ps_games:
        ...
        st.caption("✅ v2.4: Correct decimal handling + USD conversion + Fixed EU Steam markets")
        ...
        if ps_results:
            ...
            if DEBUG_MODE:
                st.markdown("**📊 v2.4 Currency Handling Results:**")
                ...
                if len(usd_prices) > 0:
                    ...
                    if min_usd < 40:
                        st.warning(f"⚠️ Some prices below $40 USD - may need correction")
                    elif min_usd > 150:
                        st.warning(f"⚠️ Some prices above $150 USD - may need correction")
                    else:
                        st.success("✅ USD prices in expected range ($40-$150)")
                
                if msrp_count > 0:
                    st.success(f"🎉 v2.4 found {msrp_count} MSRP prices with correct currency handling!")
                elif current_count > 0:
                    st.warning(f"⚠️ Found {current_count} prices via JSON-LD fallback")
                else:
                    st.error("❌ Price extraction failed")
                
                st.caption("📝 Check 'USD Price' column to verify conversions are correct")
