...
# v3.4 — Clean UX, autorun, proper Streamlit rebinding (fixes session_state crash)
import sys
...
class _Shim(_DummyCtx):
    def __init__(self):
        # simple dict to satisfy any session_state access during import
        self.session_state = {}
        self.sidebar = _DummyCtx()
    def set_page_config(self, *a, **k): return None

# Install shim, import v2_5, then rebind its internal 'st' to the real Streamlit
shim = _Shim()
sys.modules['streamlit'] = shim
import multiplatform_pricing_tool_v2_5 as v25  # noqa: E402
sys.modules['streamlit'] = real_st
v25.st = real_st  # <-- critical: ensure v2_5 uses real streamlit (has session_state)
st = real_st
...
st.set_page_config(page_title="Game Pricing — v3.4 (Clean UX, Autorun)", page_icon="🎮", layout="wide")

# Preloaded URLs (your comp set)
PRELOAD_STEAM = [
...
# ---------------- UI ----------------
st.title("Game Pricing Recommendation Tool — v3.4 (Xbox · Steam · PlayStation)")
...
# Inputs (preloaded; can be edited)
cols = st.columns(3)
...
# Autorun all three on first load or when URLs change (simple approach: always autorun)
