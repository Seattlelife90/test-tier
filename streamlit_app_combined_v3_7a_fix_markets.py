...
# streamlit_app_combined_v3_7a_fix_markets.py
# Focused repair: separate market lists for Xbox and PlayStation, keep Steam unchanged.
# Uses FULL PlayStation product URLs (recommended) and swaps to region-specific locales.
# Adds explicit supported-market filtering and clearer diagnostics.
...
st.set_page_config(page_title="Game Pricing Recommendation Tool — v3.7a (Xbox · Steam · PlayStation)", page_icon="🎮", layout="wide")
st.title("🎮 Game Pricing Recommendation Tool — v3.7a (Xbox · Steam · PlayStation)")
...
# ---------------- Supported markets (separate) ----------------
# Xbox: API expects market=two-letter ISO; locale is separate (language-country).
XBOX_LOCALE = {
    ...
    "BG":"bg-bg","SI":"sl-si","CY":"en-cy","IS":"is-is","SE":"sv-se","DK":"da-dk","NO":"no-no","FI":"fi-fi",
...
XBOX_MARKETS = set(XBOX_LOCALE.keys())

# PlayStation: locale slugs are slightly different from Xbox. Maintain a separate map.
PS_LOCALE = {
    # Americas
    "US":"en-us","CA":"en-ca","MX":"es-mx","AR":"es-ar","BR":"pt-br","CL":"es-cl","CO":"es-co","PE":"es-pe","UY":"es-uy",
    "CR":"es-cr","GT":"es-gt","HN":"es-hn","NI":"es-ni","PA":"en-pa","PY":"es-py","SV":"es-sv","DO":"es-do","BO":"es-bo",
    # Europe
    "GB":"en-gb","IE":"en-ie","FR":"fr-fr","DE":"de-de","ES":"es-es","IT":"it-it","PT":"pt-pt","NL":"nl-nl","BE":"nl-be","AT":"de-at",
    "CH":"de-ch","LU":"fr-lu","GR":"el-gr","HR":"hr-hr","HU":"hu-hu","CZ":"cs-cz","SK":"sk-sk","RO":"ro-ro","PL":"pl-pl",
    "BG":"bg-bg","SI":"sl-si","CY":"en-cy","IS":"is-is","SE":"sv-se","DK":"da-dk","NO":"no-no","FI":"fi-fi","TR":"tr-tr",
    # APAC / MEA
    "AU":"en-au","NZ":"en-nz","JP":"ja-jp","KR":"ko-kr","SG":"en-sg","MY":"en-my","TH":"th-th","ID":"id-id","TW":"zh-tw","HK":"en-hk",
    "IN":"en-in","IL":"en-il","SA":"en-sa","AE":"en-ae","ZA":"en-za"
}
PS_MARKETS = set(PS_LOCALE.keys())
...
COUNTRY_NAME = {
    ...
    "BG":"Bulgaria","SI":"Slovenia","CY":"Cyprus","IS":"Iceland","SE":"Sweden","DK":"Denmark","NO":"Norway","FI":"Finland",
...
PS_CCY = {
    "US":"USD","CA":"CAD","MX":"MXN","AR":"ARS","BR":"BRL","CL":"CLP","CO":"COP","PE":"PEN","UY":"UYU","CR":"CRC","GT":"GTQ","HN":"HNL","NI":"NIO","PA":"USD",
    "PY":"PYG","SV":"USD","DO":"DOP","BO":"BOB","GB":"GBP","IE":"EUR","FR":"EUR","DE":"EUR","ES":"EUR","IT":"EUR","PT":"EUR","NL":"EUR","BE":"EUR",
    "AT":"EUR","CH":"CHF","LU":"EUR","GR":"EUR","HR":"EUR","HU":"HUF","CZ":"CZK","SK":"EUR","RO":"RON","PL":"PLN","BG":"BGN","SI":"EUR","CY":"EUR","IS":"ISK",
    "SE":"SEK","DK":"DKK","NO":"NOK","FI":"EUR","AU":"AUD","NZ":"NZD","JP":"JPY","KR":"KRW","SG":"SGD","MY":"MYR","TH":"THB","ID":"IDR","TW":"TWD","HK":"HKD",
    "IN":"INR","TR":"TRY","IL":"ILS","SA":"SAR","AE":"AED","ZA":"ZAR"
}

# Vanity pricing (same as before; trimmed)
...
# Xbox fetch (explicit market set)
def _ms_cv(): 
...
def parse_xbox_price(payload: Dict[str,Any]):
    try:
        ...
        dsas = prods[0].get("DisplaySkuAvailabilities") or prods[0].get("displaySkuAvailabilities") or []
        for dsa in dsas:
...
def fetch_xbox_price(title, store_id, cc_iso):
    if cc_iso not in XBOX_MARKETS:
        return None, MissRow("Xbox", title or store_id, cc_iso, "unsupported_market")
...
def ps_swap_locale(ps_url: str, cc: str) -> str:
    loc = PS_LOCALE.get(cc.upper())
    if not loc: return ps_url  # unsupported -> leave as-is
    return re.sub(r"https://store\.playstation\.com/([^/]+)/", f"https://store.playstation.com/{loc}/", ps_url, count=1)
...
def fetch_playstation_price_by_url(title: str, ps_url: str, cc_iso: str, prefer_msrp: bool=True):
    if cc_iso not in PS_MARKETS:
        return None, MissRow("PlayStation", title or ps_url, cc_iso, "unsupported_market")
    url = ps_swap_locale(ps_url, cc_iso)
    headers = {"Referer": url, "Accept-Language": PS_LOCALE[cc_iso]}
    r=http_get(url, headers=headers, timeout=12)
    if not r or not r.text:
        return None, MissRow("PlayStation", title or ps_url, cc_iso, "no_response")
    html=r.text
    # If the product isn't in that region, PS sometimes redirects to a collection page that contains no price tokens.
    if "mfeCtaMain#offer0#displayPrice" not in html and "__NEXT_DATA__" not in html:
        return None, MissRow("PlayStation", title or ps_url, cc_iso, "not_available")
...
if run:
    ...
    markets_xbox=[m for m in markets if m in XBOX_MARKETS]
    markets_ps=[m for m in markets if m in PS_MARKETS]
    ...
    if not raw.empty:
        ...
        st.download_button("⬇️ Download CSV (combined recommendations)",
                           ...
                           file_name="aaa_tier_recommendations_xsx_steam_ps_v3_7a_fix_markets.csv",
                           mime="text/csv")

    if misses:
        st.subheader("Diagnostics (no price found)")
        dfm=pd.DataFrame([asdict(m) for m in misses])
        st.dataframe(dfm, use_container_width=True, height=300)
        st.caption("Tip: If PlayStation shows 'not_available', that US product URL doesn’t exist for that market. Use that market’s PS product (or concept) URL.")

else:
    st.info("Quick tip: Steam uses 2-letter 'cc' only; Xbox uses 'market' (cc) + locale; PlayStation uses locale slugs like 'en-hk'. This app now filters markets per platform accordingly.")
