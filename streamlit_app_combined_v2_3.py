...
# streamlit_app_combined_v2_3.py
...
# ---------- Defaults (PS pre-populated for 6 titles) ----------
DEFAULT_STEAM_ROWS = [
...
DEFAULT_PS_ROWS = [
    ...
    # concept links — will be resolved to /product/PPSA... on Validate
    {"include": True, "title": "NBA 2K26", "ps_ref": "https://store.playstation.com/en-us/concept/10014149", "edition_hint": "Standard", "scale_factor": 1.0, "weight": 1.0, "_ps_error": ""},
    {"include": True, "title": "Borderlands 4", "ps_ref": "https://store.playstation.com/en-us/concept/10000819", "edition_hint": "Standard", "scale_factor": 1.0, "weight": 1.0, "_ps_error": ""},
...
CONCEPT_ID_RE = re.compile(r"/concept/(\d+)")
...
def _resolve_ps_concept_to_product(concept_url: str, locale: str) -> Optional[str]:
    """Given a /concept/{id} page, return the best /product/ URL (prefer 'Standard')."""
    html = _fetch_html(concept_url, locale=locale)
    if not html:
        return None
    soup = BeautifulSoup(html, "html.parser")
    candidates = []
...
def fetch_playstation_price(ps_ref: str, market_iso: str, forced_title: Optional[str] = None, title_hint: Optional[str] = None, edition_hint: Optional[str] = None, prefer_msrp: bool = True) -> Tuple[Optional[PriceRow], Optional[MissRow]]:
    ...
    # Accept /concept/… and resolve to a product first (canonicalize with US to be safe)
    if original_ref.startswith("http") and "/concept/" in original_ref:
        resolve_locale, _ = ps_market_meta("US")
        resolved = _resolve_ps_concept_to_product(original_ref, resolve_locale)
        if resolved:
            original_ref = resolved
...
def validate_and_fill_steam_rows(rows: List[dict]) -> List[dict]:
    # Basic US lookup to fill missing names and sanity check appids.
    out = []
    for r in rows:
        appid = str(r.get("appid") or "").strip()
        if not appid:
            r["_steam_error"] = "missing appid"; out.append(r); continue
        data = None
        try:
            req = requests.get(STEAM_APPDETAILS, params={"appids": appid, "cc": "US", "l":"en"}, headers=UA, timeout=12)
            j = req.json().get(appid, {})
            if j.get("success"): data = j.get("data")
        except Exception:
            pass
        if not data: r["_steam_error"] = "not found"
        else:
            r["_steam_error"] = ""
            if not r.get("title"): r["title"] = data.get("name")
        out.append(r)
    return out
def validate_and_fill_xbox_rows(rows: List[dict]) -> List[dict]:
    out = []; headers = {"MS-CV": _ms_cv(), "Accept": "application/json"}
    for r in rows:
        sid = str(r.get("store_id") or "").strip()
        if not sid:
            r["_xbox_error"] = "missing id"; out.append(r); continue
        try:
            resp = requests.get(DISPLAYCATALOG_URL, params={"bigIds": sid, "market": "US", "languages": "en-US", "fieldsTemplate": "Details"}, headers=headers, timeout=12)
            j = resp.json() if resp.status_code == 200 else {}
            products = j.get("Products") or j.get("products") or []
            if not products: r["_xbox_error"] = "not found"
            else:
                r["_xbox_error"] = ""
                if not r.get("title"):
                    lp = (products[0].get("LocalizedProperties") or products[0].get("localizedProperties") or [])
                    if lp and lp[0].get("ProductTitle"): r["title"] = lp[0]["ProductTitle"]
        except Exception:
            r["_xbox_error"] = "lookup error"
        out.append(r)
    return out
def validate_and_fill_ps_rows(rows: List[dict]) -> List[dict]:
    out = []; locale, _ = ps_market_meta("US")  # canonicalize to US during validate
    for r in rows:
        ...
        if ref and ref.startswith("http"):
            if "/concept/" in ref:
                resolved = _resolve_ps_concept_to_product(ref, locale)
                ref = resolved or ref
...
with st.sidebar:
    ...
    with c2:
        if st.button("➕ Add Steam row"): st.session_state.steam_rows.append({"include": True, "title": "", "appid": "", "scale_factor": 1.0, "weight": 1.0, "_steam_error": ""}); st.rerun()
    ...
    with d2:
        if st.button("➕ Add Xbox row"): st.session_state.xbox_rows.append({"include": True, "title": "", "store_id": "", "scale_factor": 1.0, "weight": 1.0, "_xbox_error": ""}); st.rerun()
    ...
    ps_df = st.data_editor(pd.DataFrame(st.session_state.ps_rows), key="ps_editor", num_rows="dynamic", use_container_width=True,
        column_config={
            "include": st.column_config.CheckboxColumn(default=True),
            "title": st.column_config.TextColumn(),
            "ps_ref": st.column_config.TextColumn(help="Paste a PS Store URL (/product/… or /concept/…) or a PPSA id. Leave blank to resolve via title."),
...
def fx_view(df: pd.DataFrame, label: str) -> pd.DataFrame:
    ...
    if us_row.empty:  # can't compute diffs without US row
        return pd.DataFrame()
    ...
    if us_usd is not None:
        ...
        out["PctDiff"] = [None if pd.isna(v) or us_usd==0 else f"{round((v/us_usd - 1.0)*100.0, 0)}%" for v in out["USDPrice"]]
...
if run:
    ...
    if not raw_df.empty:
        ...
        st.subheader("Raw Basket Rows (after scaling)"); st.dataframe(raw_df, use_container_width=True)
        ...
        fx_x = fx_view(reco_xbox, "Xbox")
        fx_s = fx_view(reco_steam, "Steam")
        fx_p = fx_view(reco_ps, "PlayStation")

        if not fx_x.empty: st.subheader("Xbox Regional Pricing Recommendation"); st.dataframe(fx_x, use_container_width=True)
        if not fx_s.empty: st.subheader("Steam Regional Pricing Recommendation"); st.dataframe(fx_s, use_container_width=True)
        if not fx_p.empty: st.subheader("PlayStation Regional Pricing Recommendation"); st.dataframe(fx_p, use_container_width=True)
    ...
    if misses:
        ...
        st.subheader("Diagnostics (no price found)"); st.dataframe(miss_df, use_container_width=True)
else:
