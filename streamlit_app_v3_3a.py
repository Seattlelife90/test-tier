...
# v3.3a — Clean UX, no context-manager API calls (avoids st.container/with)
import sys
import streamlit as real_st  # type: ignore

# --- Import v2_5 with shim to suppress its UI ---
class _DummyCtx:
...
import multiplatform_pricing_tool_v2_5 as v25  # uses shimmed streamlit
...
st.set_page_config(page_title="Game Pricing — v3.3a (Clean UX)", page_icon="🎮", layout="wide")
...
st.title("Game Pricing Recommendation Tool — v3.3a (Xbox · Steam · PlayStation)")
st.caption("Paste URLs only. Each platform uses its own siloed markets & currencies from v2.5 (no cross‑talk).")

# Simple inputs without context-managers
st.subheader("Add Games by URL")
cols = st.columns(3)
steam_urls = cols[0].text_area("Steam URLs", height=140, placeholder="https://store.steampowered.com/app/...")
xbox_urls  = cols[1].text_area("Xbox URLs", height=140, placeholder="https://www.xbox.com/en-us/games/store/...")
ps_urls    = cols[2].text_area("PlayStation URLs", height=140, placeholder="https://store.playstation.com/en-us/product/...")

btn_cols = st.columns([1,1,1,3])
run_all   = st.button("Run All", use_container_width=True)
run_steam = btn_cols[0].button("Run Steam")
run_xbox  = btn_cols[1].button("Run Xbox")
run_ps    = btn_cols[2].button("Run PlayStation")
show_diag = btn_cols[3].checkbox("Show diagnostics", value=False)

def parse_list(block: str, platform_filter: str):
    items = []
