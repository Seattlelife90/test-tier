...
# ----------------------------------------
# PlayStation Pricing Scraper (v1.8.1)
# ----------------------------------------
# Fix for "everything became ZAR 87":
# - Only scan for currency symbols that match the EXPECTED currency.
# - Require the symbol to be adjacent to digits (e.g., "₱3,899", "R 1,099", "25 990 Ft").
# - Restrict numeric keys to price-like names (no generic "value"/"amount").
# - Keep deep Next.js scan + JSON-LD/meta fallbacks.
...
# Currency symbol map (no ambiguous single letters except when paired with digits via regex)
CURR_SYMBOLS = {
    "USD": ["$", "USD"],
    "EUR": ["€", "EUR"],
    "GBP": ["£", "GBP"],
    ...
    "BGN": ["лв.", "BGN"],
    "TRY": ["₺", "TRY"],
    "ZAR": ["R", "ZAR"],   # we'll only accept if 'R' is directly next to digits
    "NOK": ["kr", "NOK"],
    "SEK": ["kr", "SEK"],
    "DKK": ["kr", "DKK"],
    "ISK": ["ISK"],
...
def symbol_adjacent_to_digits(text: str, sym: str) -> bool:
    # match sym before or after digits with optional spaces, not part of a word
    pattern = rf"(?<![A-Za-z]){re.escape(sym)}\s*\d|\d\s*{re.escape(sym)}(?![A-Za-z])"
    return re.search(pattern, text) is not None

def next_json_candidates(obj: Any, expected: Optional[str]) -> List[Tuple[Optional[float], Optional[str], str]]:
    out: List[Tuple[Optional[float], Optional[str], str]] = []
    price_keys = {"discountedPrice","finalPrice","current","basePrice","originalPrice","price","strikethroughPrice","nonPlusPrice","plusPrice","regularPrice","displayPrice"}
    ...
    def walk(node: Any):
        if isinstance(node, dict):
            ...
            for k, v in node.items():
                ...
                # symbol-based detection ONLY for expected currency
                if expected and isinstance(v, str):
                    for sym in CURR_SYMBOLS.get(expected, []):
                        if symbol_adjacent_to_digits(v, sym):
                            amt = parse_localized_amount(v)
                            if amt is not None:
                                out.append((amt, expected, f"{k}:sym"))
...
def choose_best_amount(cands: List[Tuple[Optional[float], Optional[str], str]], expected: Optional[str]) -> Tuple[Optional[float], Optional[str]]:
    ...
    for amt, cur, src in cands:
        ...
        if expected and cur and cur.upper() == expected.upper():
            score += 10
        if src in {"discountedPrice","finalPrice","current","price","plusPrice","regularPrice"}:
            score += 2
        if score > best[2]:
            best = (amt, cur, score)
...
def extract_from_next(html: str, expected_currency: Optional[str]) -> Tuple[Optional[str], Optional[float], Optional[float], Optional[str]]:
    ...
    try:
        ...
        pass
    cands = next_json_candidates(nxt, expected_currency)
    cur_val, cur_code = choose_best_amount(cands, expected_currency)
    msrp_val = None
    base = [a for a in cands if a[2] in ("basePrice","originalPrice","strikethroughPrice","regularPrice") and (not expected_currency or (a[1] and a[1].upper()==expected_currency.upper()))]
    if base:
        msrp_val = base[0][0]
        ...
        for obj in objs:
            ...
            if {"product","videogame","offer"} & types or "offers" in obj:
                ...
                if isinstance(offers, dict):
                    ...
                    if amt is not None and (not expected_currency or (currency and currency.upper()==expected_currency.upper())):
...
st.title("🎮 PlayStation Store — Regional Pricing Pull (MVP v1.8.1)")
st.caption("Fixes incorrect ZAR/87 issue by tightening currency detection and symbol scanning.")
...
if st.button("Pull Prices"):
    if not product_ref.strip() and not title_hint.strip():
        ...
        st.markdown("**Notes**")
        st.markdown("- If a region shows **None**, the SKU may be unavailable there or require cookies/headers to view pricing.")
