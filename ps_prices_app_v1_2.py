...
# ----------------------------------------
# Parsing order (to avoid junk numbers like "300" / "1600"):
...
# We DO NOT fall back to "grab first two numbers" anymore.
#
# Regions covered:
#   USD (en-us, US), GBP (en-gb, GB), EUR (de-de, DE),
#   JPY (ja-jp, JP), BRL (pt-br, BR), CAD (en-ca, CA)
...
def parse_json_ld(html: str) -> Tuple[Optional[str], Optional[float], Optional[str]]:
    ...
    Returns (title, current_price, currency_code). MSRP usually not present here.
    """
    soup = BeautifulSoup(html, "html.parser")
...
st.title("🎮 PlayStation Store — Regional Pricing Pull (MVP v1.2)")
...
if st.button("Pull Prices"):
    if not product_ref.strip() and not title_hint.strip():
        ...
        st.markdown(
            "- Uses only structured sources to avoid random numbers (Next.js JSON, JSON-LD, meta tags).  
"
            "- If MSRP is missing but current price exists, MSRP is set to current (0% discount).  
"
            "- Some products/pages may hide price without locale/cookie; if a region returns None, the SKU may be unavailable there."
        )
