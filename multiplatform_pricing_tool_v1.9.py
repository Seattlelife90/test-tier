# multiplatform_pricing_tool_v1.9.py
# CRITICAL FIX: Correct PlayStation price field parsing based on actual API structure
# - Handles string prices: "$79.99" format
# - Uses basePrice (MSRP) over discountedPrice
# - Handles discountedValue numeric fields (6999 = $69.99)
# - Special handling for multi-SKU hub pages (Call of Duty)
...
st.set_page_config(
    page_title="Unified Game Pricing Tool v1.9",
    page_icon="🎮",
    layout="wide"
)
...
def _num(x: Any) -> Optional[float]:
    """
    Enhanced number converter that handles PlayStation's string formats
    Examples: "$79.99" → 79.99, "€55.99" → 55.99, 6999 → 69.99 (if it's a Value field)
    """
    try:
        if x is None:
            return None
        ...
        s = s.replace("₹", "").replace("R$", "").replace("CAD", "").replace("USD", "")
        s = s.replace(",", "").strip()
...
def _num_from_value(x: Any) -> Optional[float]:
    """
    Convert PlayStation 'Value' fields like discountedValue: 6999 → 69.99
    """
    try:
        if x is None:
            return None
        val = float(x)
        # PlayStation stores cents: 6999 = $69.99
        if val >= 100:
...
# ============================================================================
# PLAYSTATION - v1.9 CORRECT FIELD PARSING
# ============================================================================
...
def deep_search_for_prices(obj: Any, path: str = "root") -> List[Tuple[str, dict]]:
    """
    Recursively search for price objects
    Looking for objects with: basePrice, discountedPrice, baseValue, discountedValue, etc.
    """
    results = []
    
    if isinstance(obj, dict):
        # Check for PlayStation's actual price field names
        price_indicators = {
            "basePrice", "discountedPrice", "discountedValue", "baseValue",
            "currencyCode", "campaignId", "applicability"
        }
        
        # If this object has at least 2 price indicators, it's likely a price object
        matches = sum(1 for key in price_indicators if key in obj)
        if matches >= 2:
...
def extract_price_from_ps_object(price_obj: dict) -> Tuple[Optional[float], str, str]:
    """
    Extract price from PlayStation price object using CORRECT field names
    Based on dev tools inspection showing: basePrice="$79.99", discountedPrice="$55.99", discountedValue=5599
    ...
    # PRIORITY 1: basePrice (MSRP) - This is always the full price
    base_str = price_obj.get("basePrice")
    base_price = _num(base_str)
    if base_price and base_price > 0:
        debug_parts.append(f"basePrice={base_str}")
        disc_str = price_obj.get("discountedPrice")
        if disc_str:
            debug_parts.append(f"discountedPrice={disc_str}")
        debug_parts.append("✓used_basePrice_MSRP")
        return base_price, "MSRP", "|".join(debug_parts)
    
    # PRIORITY 2: baseValue (numeric version of basePrice, e.g., 7999 = $79.99)
    base_val = price_obj.get("baseValue")
    if base_val:
        base_price = _num_from_value(base_val)
        if base_price and base_price > 0:
            debug_parts.append(f"baseValue={base_val}")
            debug_parts.append("✓used_baseValue_MSRP")
            return base_price, "MSRP", "|".join(debug_parts)
    
    # PRIORITY 3: If no basePrice, use discountedPrice (but mark as Sale)
    disc_str = price_obj.get("discountedPrice")
    disc_price = _num(disc_str)
    if disc_price and disc_price > 0:
        debug_parts.append(f"discountedPrice={disc_str}")
        debug_parts.append("⚠used_discountedPrice_NO_BASE")
        return disc_price, "Sale Price", "|".join(debug_parts)
    
    # PRIORITY 4: discountedValue (numeric version)
    disc_val = price_obj.get("discountedValue")
    if disc_val:
        disc_price = _num_from_value(disc_val)
        if disc_price and disc_price > 0:
            debug_parts.append(f"discountedValue={disc_val}")
            debug_parts.append("⚠used_discountedValue_NO_BASE")
            return disc_price, "Sale Price", "|".join(debug_parts)
    
    # No valid price found
    keys = list(price_obj.keys())
    return None, "Unknown", f"no_valid_price|keys={keys}"

def from_next_json_deep(next_json: dict) -> Tuple[Optional[str], Optional[str], Optional[float], str, Optional[str], str]:
    """
    Extract price from Next.js JSON using deep search with correct PlayStation field names
    ...
    try:
        ...
        # Sort by path depth (shallower = more likely correct)
        price_candidates.sort(key=lambda x: x[0].count('.'))
        
        # Try each price object
        for path, price_obj in price_candidates:
            price, price_type, extract_debug = extract_price_from_ps_object(price_obj)
        ...
        # Found objects but no valid prices
        paths = [p[0] for p in price_candidates[:3]]
        return title, pid, None, "Unknown", None, f"found_{len(price_candidates)}_objects_no_prices|paths={','.join(paths)}"
...
def fetch_ps_price(product_id: str, country: str, title: str) -> Optional[PriceData]:
    """Fetch PlayStation price with correct field parsing"""
    ...
    try:
        ...
        # Try Next.js JSON with correct field parsing
        ...
        if nxt:
            ...
            if price is not None and price > 0:
                ...
                print(f"[PSN {country}] {title}: {price} {final_currency} ({price_type}) - {parse_debug[:100]}")
                return PriceData("PlayStation", title, country, final_currency, price, None, None, price_type, "next_json", full_debug)
            else:
                debug_log.append(f"nextjs_no_price:{parse_debug[:80]}")
        else:
        ...
        if price2 is not None and price2 > 0:
            ...
            print(f"[PSN {country}] {title}: {price2} {final_currency} (JSON-LD)")
            return PriceData("PlayStation", title, country, final_currency, price2, None, None, "Current", "json_ld", full_debug)
        else:
            debug_log.append(f"json_ld_no_price:{ld_debug[:50]}")
        ...
        full_debug = "|".join(debug_log + ["all_methods_failed"])
...
st.title("🎮 Unified Multi-Platform Game Pricing Tool v1.9")
st.caption("✅ CRITICAL FIX: Correct PlayStation field parsing - basePrice (MSRP) over discountedPrice")

if DEBUG_MODE:
    st.info("🔍 **v1.9 Changes:** Handles PlayStation's string prices ('$79.99'), uses basePrice/baseValue for MSRP")
    ...
    if not steam_games and not xbox_games and not ps_games:
        ...
        if ps_results:
            ...
            if DEBUG_MODE:
                ...
                # Show MSRP vs Sale breakdown
                msrp_count = (ps_df['Price Type'] == 'MSRP').sum()
                sale_count = (ps_df['Price Type'] == 'Sale Price').sum()
                st.markdown(f"**✅ MSRP Prices: {msrp_count} | ⚠️ Sale Prices: {sale_count}**")
                st.caption("📊 Debug Info shows exact field used (basePrice vs discountedPrice)")
