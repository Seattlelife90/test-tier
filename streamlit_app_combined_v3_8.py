...
# streamlit_app_combined_v3_8.py
# v3.8 highlights
# - PlayStation edition targeting: prefer Standard or Cross-Gen over Deluxe/Ultimate/Super/Bundle
# - If page is a non-preferred edition, follow the "Standard"/"Cross-Gen" product link and price that
# - MSRP-first parsing kept (Prefer MSRP -> choose highest price found on page)
# - Xbox request headers/params hardened
# - Progress bar + Quick Run + Vanity pricing + new "edition" column in Raw rows
...
st.set_page_config(page_title="Game Pricing Recommendation Tool — v3.8", page_icon="🎮", layout="wide")
st.title("🎮 Game Pricing Recommendation Tool — v3.8 (Xbox · Steam · PlayStation)")
...
def fetch_xbox_price(title, store_id, cc_iso):
    ...
    headers={
        ...
        "X-Client-Id":"pricing-tool/1.0"
    }
    r=http_get("https://storeedgefd.dsx.mp.microsoft.com/v9.0/sdk/products",
               params={"bigIds":store_id,"market":cc_iso.upper(),"locale":loc}, headers=headers, timeout=8, retries=1)
    try:
        if hasattr(r,"json"):
...
EXCLUDE_EDITIONS = ["deluxe", "ultimate", "super", "bundle"]
PREFER_EDITIONS  = ["standard", "cross-gen"]  # as requested
...
def _score_edition(text:str)->int:
    t=(text or "").lower()
    score=0
    if any(k in t for k in PREFER_EDITIONS): score+=100
    if any(k in t for k in EXCLUDE_EDITIONS): score-=100
    return score
...
def fetch_playstation_price(ps_ref, cc_iso, title=None, prefer_msrp=True):
    ...
    for loc in locales_try:
        for cand in pids_try:
            ...
            # If current page looks like Deluxe/Ultimate etc., try to locate a Standard/Cross-Gen link
            ...
            if _score_edition(label_for_score) < 0:
                ...
                if target_href and target_href != url:
                    ...
                    if html2:
                        ...
                        values=_collect_prices_from_html(html2)
                        if b2 is None and values: b2=max(values)
                        if d2 is None and values: d2=min(values)
                        msrp = b2 if b2 is not None else msrp
                        price = d2 if d2 is not None else price
                        curr  = curr2 or curr
                        edition_label = lab2 or target_label or "Standard"
                        url = target_href  # use standard/cross-gen page as source

            # Fallback scan on chosen html
            values=_collect_prices_from_html(html)
            if values:
                if msrp is None: msrp=max(values)
                if price is None: 
                    small=min(values)
                    price = small if (msrp is None or small <= msrp) else None
...
if run:
    ...
    if not raw.empty:
        ...
        st.download_button("⬇️ Download CSV (combined recommendations)", merged.to_csv(index=False).encode("utf-8"),
                           file_name="aaa_tier_recommendations_xsx_steam_ps_v3_8.csv", mime="text/csv")
